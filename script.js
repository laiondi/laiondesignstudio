...
// =========================================================
// HOME DINÁMICO SEGÚN LA HORA LOCAL
// 06:00–12:59  → día
// 13:00–18:59  → tarde
// 19:00–05:59  → noche
// =========================================================
(function initTimeBasedHome() {
    ...
    Object.values(homeImages).forEach((src) => {
   ...
   function getPeriod(hour) {
    ...
    if (hour >= 23 && hour < 24) return "afternoon";
    return "night";
}

    function updateHomeByTime() {
        const now = new Date();
        const period = getPeriod(now.getHours());

        if (home.dataset.period !== period) {
            home.dataset.period = period;
        }
    ...
    updateHomeByTime();
    // Actualizar una vez por segundo mantiene el reloj vivo y también detecta
    // automáticamente cuando se cruza de día → tarde → noche.
    window.setInterval(updateHomeByTime, 1000);
})();
